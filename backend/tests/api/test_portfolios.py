...
async def test_portfolio_lifecycle_and_accounting():
    async with async_session_maker() as db_session:
        user = User(id=9901, email=f"test_{uuid.uuid4()}@example.com", password_hash="pw", is_active=True)
        db_session.add(user)
        inst = Instrument(id=9901, symbol=f"TEST_{uuid.uuid4().hex[:4]}", name="Test", exchange="BIST", instrument_type=InstrumentType.STOCK)
        db_session.add(inst)
...
async def test_portfolio_idor():
    async with async_session_maker() as db_session:
        user1 = User(id=9902, email=f"test_{uuid.uuid4()}@example.com", password_hash="pw", is_active=True)
        user2 = User(id=9903, email=f"test_{uuid.uuid4()}@example.com", password_hash="pw", is_active=True)
