...
from app.db.models import Portfolio, PortfolioTransaction, PositionLifecycle, Instrument, User, LifecycleHealthState, LifecycleAction, PositionLifecycleSnapshot
...
async def test_lifecycle_scenario_episode_semantics(client: AsyncClient, auth_headers: dict, setup_lifecycle_portfolio, monkeypatch):
    ...
    async with async_session_maker() as db:
        tx2 = PortfolioTransaction(portfolio_id=port.id, instrument_id=inst.id, transaction_type="SELL", quantity=Decimal("5"), executed_at=datetime.now(timezone.utc))
        db.add(tx2)
        await db.commit()
    ...
    async with async_session_maker() as db:
        tx3 = PortfolioTransaction(portfolio_id=port.id, instrument_id=inst.id, transaction_type="SELL", quantity=Decimal("5"), executed_at=datetime.now(timezone.utc))
        db.add(tx3)
        await db.commit()
    ...
    async with async_session_maker() as db:
        tx4 = PortfolioTransaction(portfolio_id=port.id, instrument_id=inst.id, transaction_type="BUY", quantity=Decimal("10"), executed_at=datetime.now(timezone.utc))
        db.add(tx4)
        await db.commit()
