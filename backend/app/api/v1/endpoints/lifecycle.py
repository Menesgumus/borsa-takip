...
from app.db.models import User, Portfolio, PositionLifecycle, PositionLifecycleSnapshot, Instrument, PortfolioTransaction, AssetClass
from app.schemas.lifecycle import PositionLifecycleDTO, LifecycleSummaryDTO, LifecycleSnapshotDTO
...
from app.services.lifecycle_engine import evaluate_lifecycle_for_instrument, process_position_lifecycle, LifecycleAction, LifecycleHealthState
...
async def evaluate_lifecycle(
    ...
    for p_dict in val.positions:
        ...
        # Evaluate Engine (Adapter)
        # We need the current USDTRY if US_EQUITY for fx_rate, but since it's just for context key, 1.0 is fine if we don't have it here. 
        # Actually evaluate_portfolios doesn't return fx_rate directly. We will pass 1.0.
        decision, market_key, context_key_partial, is_evaluable = await evaluate_lifecycle_for_instrument(
            db, current_user, portfolio, inst, qty, weight, cash_balance, limit_pct, is_valuation_complete, Decimal("1.0")
        )
        ...
        if dto.suggested_reduce_quantity is not None:
            ...
            fx = Decimal(str(p_dict.get("fx_rate") or "1"))
