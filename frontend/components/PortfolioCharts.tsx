  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        ...
        <div className="bg-white p-3 border border-slate-200 shadow-lg rounded-lg text-sm min-w-[140px] z-50">

          <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-100">

            <div className="w-3 h-3 rounded-full" style={{ backgroundColor: colorForSymbol(data.name) }}></div>

            <p className="font-bold text-navy-900">{data.name}</p>
          ...
          <div className="space-y-1">
            ...
            {data.isCash ? (
              ...
              <div className="flex justify-between gap-4"><span className="text-slate-500">Tutar:</span> <span className="font-bold text-navy-900">{formatTry(data.value)}</span></div>

            ) : (

              <>

                <div className="flex justify-between gap-4"><span className="text-slate-500">Değer:</span> <span className="font-bold text-navy-900">{formatTry(data.value)}</span></div>

                <div className="flex justify-between gap-4"><span className="text-slate-500">Adet:</span> <span className="font-medium text-navy-900">{formatQuantity(data.quantity)}</span></div>

              </>

            )}

            <div className="flex justify-between gap-4"><span className="text-slate-500">Pay:</span> <span className="font-bold text-navy-900">{formatPercent(percentage)}</span></div>
          ...
          {allocationData.length === 0 ? (
            ...
            <div className="relative w-full h-full min-h-[350px]">
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                <RechartsTooltip content={<CustomTooltip />} position={{ x: 0, y: 0 }} wrapperStyle={{ zIndex: 100 }} />
              ...
              <div className="absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center pointer-events-none text-center w-[130px]">
                ...
                <span className="text-sm font-bold text-navy-900 leading-tight tracking-tight whitespace-nowrap tabular-nums overflow-hidden text-ellipsis max-w-[120px]">
