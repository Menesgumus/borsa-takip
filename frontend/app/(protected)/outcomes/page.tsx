...
import { LineChart, ShieldAlert, Zap, WifiOff } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto p-4 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">Geçmiş kararların ileriye dönük gerçekleşen getirileri.</p>
        </div>
        <button 
          onClick={() => {
            if(!isOnline) alert("İnternet bağlantısı olmadan worker tetiklenemez.");
            else triggerMutation.mutate();
          }} 
          disabled={triggerMutation.isPending || !isOnline}
          className={`px-4 py-2 rounded-lg flex items-center justify-center gap-2 min-h-[44px] w-full md:w-auto ${
            isOnline ? 'bg-indigo-600 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'
          }`}
        >
      ...
      {!isOnline && (
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-lg flex gap-3 shadow-sm">
          ...
          <div>
            ...
            <p className="text-sm">Çevrimdışısınız. Yeni analizler başlatılamaz ve verileriniz eski olabilir.</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="p-4 overflow-x-auto">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4 flex gap-3 text-yellow-800 text-sm">
            ...
            <div>
              <strong>Otomatik Terfi Durduruldu:</strong> Gerçek veri seti limiti nedeniyle otomatik terfi engellenmiştir. (Shadow Mode).
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        {loadingOutcomes ? (
          ...
          <>
            ...
            {/* Mobile View for Outcomes */}
            <div className="md:hidden divide-y divide-gray-100">
              {recentOutcomes?.map((out: any) => (
                <div key={out.id} className="p-4">
                  <div className="font-bold text-sm mb-2">Karar #{out.decision_id}</div>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-gray-50 p-2 rounded">
                      <div className="text-gray-500">T+1</div>
                      <div className="font-mono">{out.return_t1}%</div>
                    </div>
                    <div className="bg-gray-50 p-2 rounded">
                      <div className="text-gray-500">T+5</div>
                      <div className="font-mono">{out.return_t5}%</div>
                    </div>
                    <div className="bg-gray-50 p-2 rounded">
                      <div className="text-gray-500">T+20</div>
                      <div className="font-mono">{out.return_t20}%</div>
                    </div>
                    <div className="bg-gray-50 p-2 rounded">
                      <div className="text-gray-500">T+60</div>
                      <div className="font-mono">{out.return_t60}%</div>
                    </div>
                  </div>
                </div>
              ))}
