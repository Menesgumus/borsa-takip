...
export default function SettingsPage() {
  ...
  useEffect(() => {
    // Load from local storage for now since backend doesn't have these fields on User model yet
    const storedRisk = localStorage.getItem('bt_risk_tolerance');
    const storedLevel = localStorage.getItem('bt_explanation_level');
    if (storedRisk) setRiskTolerance(storedRisk);
    if (storedLevel) setExplanationLevel(storedLevel);
  }, []);

  const handleSave = () => {
    localStorage.setItem('bt_risk_tolerance', riskTolerance);
